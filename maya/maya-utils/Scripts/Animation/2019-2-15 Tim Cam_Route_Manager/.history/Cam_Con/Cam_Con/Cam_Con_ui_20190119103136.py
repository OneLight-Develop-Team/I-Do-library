...
from Cam_Con import Cam_Con

class Cam_Con_UI_Interface(Cam_Con):
    def __init__(self,dock="undock"):
        ...
        super(Cam_Con_UI_Interface,self).__init__()
    ...
    def Dockable_Window_Fun(self,dock="undock",save=True):
        ...
        global Cam_Con_UI 
        Cam_Con_UI = Cam_Con_UI_Interface(dock=dock)
...
def main():
    ...
    try:
        if cmds.window(Cam_Con_UI.undockWindow,query=True,exists=True) :
            cmds.evalDeferred("cmds.deleteUI(\"" + Cam_Con_UI.undockWindow + "\")")
            # cmds.deleteUI(Cam_Con_UI.undockWindow)
    except:
        pass

    try:
        if cmds.dockControl(Cam_Con_UI.dockControl,query=True,exists=True) :
            cmds.evalDeferred("cmds.deleteUI(\"" + Cam_Con_UI.dockControl + "\")")
            # cmds.deleteUI(Cam_Con_UI.dockControl)
    except:
        pass

    try:
        if mel.eval("getApplicationVersionAsFloat;")>=2017:
            if cmds.workspaceControl(Cam_Con_UI.workspaceCtrl,query=True,exists=True) :
                cmds.deleteUI(Cam_Con_UI.workspaceCtrl)
    except:
        pass

    Cam_Con_UI = Cam_Con_UI_Interface(dock="undock")
