        ...
        for i,child in enumerate(self.Cam_Item_Widget.Item_Layout.children()):
            if i != 0:
                if child.geometry().contains(e.pos().x(),e.pos().y()-60):
                    ...
                    print self.Cam_Item_Widget.Item_Layout.geometry()
                    # child.setStyleSheet("#%s{border: 3px solid blue}" % child.objectName())
                    child.setStyleSheet("#%s{background: 3px solid blue}" % child.objectName())
                else:
                    pass
                    # child.setStyleSheet("#%s{}"% child.objectName())
