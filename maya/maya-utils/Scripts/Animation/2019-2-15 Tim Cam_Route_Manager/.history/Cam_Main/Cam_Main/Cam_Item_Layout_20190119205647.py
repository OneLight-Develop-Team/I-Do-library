...
class Cam_Item_Layout(form_class,base_class):
    ...
    def Batch_Constraint_Fn(self):
        ...
        for i,child in enumerate(ChildrenList):
            if i != 0:
                cmds.setAttr("%s.%s" % (pnCns,Attr_List[i]),0)
                try :
                    child.Cam_Con_CB.stateChanged.disconnect()
                except:
                    pass
                child.Cam_Con_CB.stateChanged.connect(partial(self.Cam_Con_CB_Fn,pnCns,child,Attr_List,i))
    
    def Cam_Con_CB_Fn(self,CB,Con,Attr_List,num,state):
        ChildrenList = self.Item_Layout.children()
        for i,child in enumerate(ChildrenList):
            if i != 0:
                if child != CB:
                    ...
                    cmds.setAttr("%s.%s" % (Con,Attr_List[i]),0)

        if state == 2:
            CB.Cam_Con_CB.setChecked(True)
            cmds.setAttr("%s.%s" % (Con,Attr_List[num]),1)
        else:
            CB.Cam_Con_CB.setChecked(False)
            cmds.setAttr("%s.%s" % (Con,Attr_List[num]),0)
