...
class Cam_Attribute_Panel(form_class,base_class):
    ...
    def Add_Loc_Pick_Fun(self):
        if len(cmds.ls(sl=True)) > 0:
            self.Add_Loc_LE.setText(cmds.ls(sl=True)[0])
            try :
                self.Add_Loc_Get.clicked.disconnect()
            except:
                pass
            self.Add_Loc_Get.clicked.connect(partial(self.Select_Loc_Fun,cmds.ls(sl=True)[0]))
        else :
    ...
    def Select_Loc_Fun(self,selectTarget):
        if selectTarget != "":
