...
class Cam_Item(form_class,base_class):
    def __init__(self,parent):
        ...
        parent.Item_Layout.layout().insertWidget(0,self)
        TotalCount = len(parent.Item_Layout.children())
        for i,child in enumerate(parent.Item_Layout.children()):
            if i != 0:
                child.Cam_Num_Label.setText(u"镜头%s" % (TotalCount-i))
                child.setObjectName("Cam_Item_%s" % (TotalCount-i))
                self.Cam_LE.setObjectName("Cam_Item_%s" % (TotalCount-i))
        ...
        for i,child in enumerate(ChildrenList):
            if i != 0:
                ...
                if child == self:
                    ...
                    child.Cam_Num_Label.setText(u"镜头%s" % (TotalCount-i-1))
                    child.setObjectName("Cam_Item_%s" % (TotalCount-i-1))
