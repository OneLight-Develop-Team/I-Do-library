...
class Cam_Attribute_Panel(form_class,base_class):
    ...
    def Check_Selection(self):
        ...
        # Note 检查是否选择好所有的东西
        """
        Crv_Check = self.Add_Crv_LE.text() != ""
        Loc_Check = self.Add_Loc_LE.text() != ""
        Path_Check = self.Add_Motion_Path_LE.text() != ""
        Cam_Base_Cehck = self.MainWindow.Cam_Item_Widget.Attr["Add_Crv_LE"] != ""
        Ex_Crv_Check = cmds.objExists(self.Add_Crv_LE.text())
        Ex_Loc_Check = cmds.objExists(self.Add_Loc_LE.text())
        Ex_Path_Check = cmds.objExists(self.Add_Motion_Path_LE.text())
        Ex_Cam_Base_Cehck = cmds.objExists(self.MainWindow.Cam_Item_Widget.Attr["Add_Crv_LE"])
        ...
        # if Crv_Check and Loc_Check and Path_Check and Cam_Base_Cehck and End_Time_Cehck and Start_Time_Cehck:
        if Crv_Check and Loc_Check and Path_Check and Cam_Base_Cehck :
            return True
        else:
            return False
