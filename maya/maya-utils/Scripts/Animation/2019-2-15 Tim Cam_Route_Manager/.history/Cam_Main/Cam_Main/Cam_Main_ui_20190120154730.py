...
class Cam_Main_UI_Interface(Cam_Main):
    ...
    def Help_Instruction_Fun(self):
        ...
        cmds.textField(tx=u"1.点击添加镜头可以添加任意镜头",ed=False,bgc=(.1,.1,.1))
        cmds.textField(tx=u"2.选择添加的镜头（点击x按钮旁边即可选择）",ed=False,bgc=(.1,.1,.1))
        cmds.textField(tx=u"3.完善右侧面板上的信息。（选中物体，然后点击右侧的按钮加选到面板上）",ed=False,bgc=(.1,.1,.1))
        cmds.textField(tx=u"4.完成所有镜头之后就可以点击左侧面板下方的按钮来完成操作",ed=False,bgc=(.1,.1,.1))
        cmds.textField(tx=u"<center>制作者：梁伟添 - 2019-1-20</center>",ed=False,bgc=(.1,.1,.1))
