    ...
    def Batch_Position_Fn(self):
        ...
        for i,child in enumerate(ChildrenList):
            if i != 0:
                ...
                cmds.setAttr("%s.tx" % CamGrp,lock=False)
                cmds.setAttr("%s.ty" % CamGrp,lock=False)
                cmds.setAttr("%s.tz" % CamGrp,lock=False)
                cmds.setAttr("%s.rx" % CamGrp,lock=False)
                cmds.setAttr("%s.ry" % CamGrp,lock=False)
                cmds.setAttr("%s.rz" % CamGrp,lock=False)
