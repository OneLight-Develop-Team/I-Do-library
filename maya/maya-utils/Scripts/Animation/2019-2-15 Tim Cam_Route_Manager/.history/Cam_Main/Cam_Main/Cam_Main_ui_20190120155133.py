...
class Cam_Main_UI_Interface(Cam_Main):
    ...
    def Help_Instruction_Fun(self):
        ...
        cmds.text(tx=u"————————————————主要用法————————————————",bgc=(1,.1,.1))
        cmds.text(tx=u"1.点击添加镜头可以添加任意镜头",bgc=(.1,.1,.1))
        cmds.text(tx=u"2.选择添加的镜头（点击x按钮旁边即可选择）",bgc=(.1,.1,.1))
        cmds.text(tx=u"3.完善右侧面板上的信息。（选中物体，然后点击右侧的按钮加选到面板上）",bgc=(.1,.1,.1))
        cmds.text(tx=u"4.完成所有镜头之后就可以点击左侧面板下方的按钮来完成操作",bgc=(.1,.1,.1))
        cmds.text(tx=u"<center>制作者：梁伟添 - 2019-1-20</center>",bgc=(.1,.1,.1))
