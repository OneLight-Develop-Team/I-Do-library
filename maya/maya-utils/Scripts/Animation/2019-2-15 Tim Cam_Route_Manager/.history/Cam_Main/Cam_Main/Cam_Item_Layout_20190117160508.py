...
class Cam_Item_Layout(form_class,base_class):
    ...
    def Item_Add_Fn(self):
        ...
        print self.Item_Layout.children()
        TotalCount = len(self.Item_Layout.children())
...
class Cam_Item(form_class,base_class):
    ...
    def Cam_Del_BTN_Fn(self):
        ...
        TotalCount = len(self.parent().Item_Layout.children())
        for i,child in enumerate(self.parent().Item_Layout.children()):
            if i != 0:
                child.Cam_Num_Label.setText(u"镜头%s" % (TotalCount-i))