...
class Cam_Main(form_class,base_class):
    ...
    def Cam_Item_Pressing_Event(self,e):
        ...
        for i,child in enumerate(self.Cam_Item_Widget.Item_Layout.children()):
            ...
            if self.Cam_Item_Widget.Cam_Base_Label.geometry().contains(e.pos().x(),e.pos().y()-40):
                ...
                self.Cam_Attribute_Widget.Add_Crv_LE.setText(Attr["Add_Crv_LE"])
                self.Cam_Attribute_Widget.Add_Motion_Path_LE.setText(Attr["Add_Motion_Path_LE"])
                self.Cam_Attribute_Widget.Add_Motion_Path_LE.setText(Attr["Add_CamGrp_LE"])
                self.Cam_Attribute_Widget.Add_Loc_LE.setText(Attr["Add_CamGrp_LE"])
                # Note 检查选择
                if self.Cam_Attribute_Widget.Check_Selection():
                    currentSelect = cmds.ls(sl=1)
                    try:
    ...
    def Save_Json_Fun(self,path=GUI_STATE_PATH):
        ...
        GUI_STATE['Cam_Base']["Attr"]["Add_CamGrp_LE"] = self.Cam_Item_Widget.Attr["Add_CamGrp_LE"]
        GUI_STATE['Cam_Base']["Attr"]["Add_Loc_LE"] = self.Cam_Item_Widget.Attr["Add_Loc_LE"]
        GUI_STATE['Cam_Base']["Attr"]["Add_Motion_Path_LE"] = self.Cam_Item_Widget.Attr["Add_Motion_Path_LE"]
        GUI_STATE['Cam_Base']["Attr"]["Name"] = self.Cam_Item_Widget.Cam_LE.text()
    ...
    def Load_Json_Fun(self,path=GUI_STATE_PATH,load=False):
        if os.path.exists(path):
            ...
            self.Cam_Item_Widget.Attr["Add_CamGrp_LE"] = GUI_STATE['Cam_Base']["Attr"]["Add_CamGrp_LE"]
            self.Cam_Item_Widget.Attr["Add_Loc_LE"] = GUI_STATE['Cam_Base']["Attr"]["Add_Loc_LE"]
            self.Cam_Item_Widget.Cam_LE.setText(GUI_STATE['Cam_Base']["Attr"]["Name"])
