...
class Cam_Attribute_Panel(form_class,base_class):
    ...
    def Check_Selection(self):
        Add_Crv_LE_Cehck = self.Add_Crv_LE.text() != ""
        Add_Loc_LE_Cehck = self.Add_Loc_LE.text() != ""
        Add_Motion_Path_LE_Cehck = self.Add_Motion_Path_LE.text() != ""
        Cam_Base_LE_Cehck = self.MainWindow.Cam_Item_Widget.Cam_Base_LE.text() != ""
        if Add_Crv_LE_Cehck and Add_Loc_LE_Cehck and Add_Motion_Path_LE_Cehck and Cam_Base_LE_Cehck:
            return True
        else:
            return False
    ...
    def Position_Fn(self):
        if self.Check_Selection():
            Base_Curve = self.MainWindow.Cam_Item_Widget.Cam_Base_LE.text()
            cmds.delete(cmds.parentConstraint( Base_Curve,self.Add_Crv_LE.text() ))
        else:
